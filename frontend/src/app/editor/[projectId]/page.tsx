...
export default function Editor() {
  ...
  // Project data
  const { data: project, isLoading: projectLoading } = useProject(projectId) as { data: ProjectResponse | undefined, isLoading: boolean };
  ...
  const handleSave = useCallback(async (isAutoSave = false) => {
    ...
    try {
      const canvasData: any = JSON.parse(exportToJSON());
      ...
      if (projectId) {
        ...
        router.replace(`/editor/${newProject.project.id}`);
      }
  ...
  }, [projectId, projectTitle, exportToJSON, exportToPNG, updateProjectMutation, createProjectMutation, router, toast]);
  ...
  const handleExport = useCallback(async (format: "png" | "jpg" | "svg" | "pdf") => {
    try {
  ...
  }, [exportToPNG, exportToSVG, projectTitle, toast]);
  ...
  }, [setBackgroundGradient, setBackgroundColor]);
